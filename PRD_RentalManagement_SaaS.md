...
| Auth | Supabase Auth (auth.get_user()) | Built-in, algorithm-agnostic token verification |
...
| PDF Export | fpdf2 (Python) | Lightweight, không cần system dependencies |
...
> **Cập nhật**: Ảnh CCCD hiện dùng `id_photo_links` (list URLs, tối đa 5 link) thay vì upload file trực tiếp (`id_front_url`/`id_back_url`). Trường `notes TEXT` đã được thêm vào bảng.
...
### 4.3 Luồng thêm người thuê và tự động liên kết tài khoản

```
1. Owner điền form "Thêm người thuê":
   - email (bắt buộc) + full_name (bắt buộc) + phone, CCCD, etc.
2. Hệ thống tự động:
   - Tạo record trong `renter_profiles`
   - Tạo record trong `invitations` với role='renter' và email đó
3. Khi người thuê đăng nhập với email đó (Google OAuth hoặc email/password):
   - `google_auth()` phát hiện invitation → gán role='renter' + liên kết organization
   - Người thuê chỉ thấy: hợp đồng, hóa đơn, và yêu cầu bảo trì của mình
```

> **Lưu ý**: Không cần gửi email mời riêng. Chỉ cần người thuê đăng nhập đúng email đã đăng ký là hệ thống tự động liên kết.
...
> **Cập nhật luồng quản lý nhân viên:**
> - Thay đổi từ "Mời nhân viên" sang "Thêm nhân viên" (tạo trực tiếp với email bắt buộc)
> - Khi thêm nhân viên: hệ thống tạo record trong bảng `invitations` với role tương ứng
> - Nhân viên tự động nhận role khi đăng nhập với email đã đăng ký
> - Danh sách nhân viên hiển thị cả active (đã đăng nhập) và pending (chờ đăng nhập)
> - Các trường bổ sung: `notes`, `address` trong profile nhân viên
...
**Auto-detection lời mời khi đăng nhập:**

Khi user mới đăng nhập qua Google OAuth, hệ thống tự động kiểm tra bảng `invitations`:
- Nếu có invitation với email trùng khớp (chưa accepted): tự động gán role và liên kết organization từ invitation
- Áp dụng cho cả staff (manager, accountant, maintenance, cleaner) VÀ renter
- Sau khi liên kết, invitation được đánh dấu `accepted_at`
- Không cần click link mời riêng - chỉ cần đăng nhập đúng email là được

> **Lưu ý**: Token verification sử dụng `supabase.auth.get_user(access_token)` thay vì decode JWT cục bộ. Đây là cách tiếp cận algorithm-agnostic, không phụ thuộc vào JWT secret.
...
> **Cập nhật**: Luồng setup/rename organization hoạt động hoàn chỉnh. User mới từ Google OAuth được redirect tới `/organization-setup` để đặt tên tổ chức. Owner có thể đổi tên tổ chức bất kỳ lúc nào qua Settings.
...
### 12.6 Ghi chú triển khai chung

| Hạng mục | Chi tiết |
|---|---|
| Ngôn ngữ UI | Toàn bộ giao diện bằng tiếng Việt |
| Chọn property/unit | Sử dụng dropdown selector (không phải nhập ID text) |
| Token verification | Sử dụng `supabase.auth.get_user()` (algorithm-agnostic, không decode JWT cục bộ) |
| Tên ứng dụng | "My Home" |
| PDF generation | fpdf2 (lightweight, không cần WeasyPrint/wkhtmltopdf) |
| Invitation flow | Áp dụng cho cả staff và renter: tạo trực tiếp với email → auto-link khi login |
